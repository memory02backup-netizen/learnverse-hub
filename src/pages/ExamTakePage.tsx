...
export default function ExamTakePage() {
  ...
  // Determine if exam is purely written (no MCQ)
  ...
  // Auto-submit handler for suspicious activity (tab switches)
  ...
  // Fullscreen exit / escape - show confirmation dialog
  const handleFullscreenExitConfirm = useCallback(() => {
    if (submittedRef.current) return;
    setShowExitConfirm(true);
  }, []);

  // Track if current question is written type
  ...
  // Exam security hook - only for MCQ exams
  ...
  // Check if user already entered this exam (entry tracking)
  useEffect(() => {
  ...
  const startExam = async () => {
    ...
    // Record exam entry in Firestore
    try {
  ...
  const handleCameraCapture = async (questionId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Re-enter fullscreen after upload (only for MCQ exams)
    if (hasMcqQuestions) {
      setTimeout(() => {
        requestFullscreen();
      }, 500);
  ...
  // If user already entered but no submission exists and exam is still live - block re-entry
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto animate-fade-in select-none">
      ...
      {!isFullscreen && started && isWrittenUploadActive && hasMcqQuestions && (
